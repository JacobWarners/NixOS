...
let 
  ...
  bibata-cursors = pkgs.stdenv.mkDerivation rec {
    ...
    src = pkgs.fetchFromGitHub {
      ...
      repo = "Bibata_Cursor";
      rev = "v${version}";
      # This hash is verified to be correct for version 2.0.6
      hash = "sha256-iLBgQ0reg8HzUQMUcZboMYJxqpKXks5vJVZMHirK48k=";
    };

    installPhase = ''
      mkdir -p $out/share/icons
      # The theme files are in the root of the source, not a 'dist' folder
...
in { 
  ...
  home.pointerCursor = {
    ...
    name = "Bibata-Modern-Classic";
    size = 24;
    gtk.enable = true; # Inherit GTK settings
    x11.enable = true; # Enable for XWayland apps
  };
  ...
  wayland.windowManager.hyprland = { 
    ...
    extraConfig = '' 
      # ... your full hyprland config is unchanged ...
      ...
      windowrulev2 = suppressevent maximize, class:.* windowrulev2 = nofocus,class:^$,title:^$,xwayland:1,floating:1,fullscreen:0,pinned:0 
    ''; 
  }; 
}

...
in {
  ...
  # This section uses your preferred 'getFrom' function structure.
  home.pointerCursor =
    let
      # We've adapted your function to build the package properly.
      getFrom = repo-owner: repo-name: repo-rev: repo-hash: theme-name: {
        gtk.enable = true;
        x11.enable = true;
        name = theme-name;
        size = 24;
        package = pkgs.stdenv.mkDerivation {
          name = "${theme-name}-cursor-theme";

          # We use fetchFromGitHub as it's more reliable for this repo.
          ...
          # The install script copies the correct theme folder.
          installPhase = ''
            mkdir -p $out/share/icons
            cp -R $src/${theme-name} $out/share/icons/
          '';
        };
      };
    in
      # We call the function with the Bibata theme details.
      getFrom
        "ful1e5"                              # Repo Owner
        "Bibata_Cursor"                       # Repo Name
        "v2.0.6"                              # Repo Version/Tag
        "sha256-iLBgQ0reg8HzUQMUcZboMYJxqpKXks5vJVZMHirK48k=" # The correct hash
        "Bibata-Modern-Classic";              # The theme folder name
  ...
  programs.vim = {
    ...
    extraConfig = ''
      syntax on; colorscheme gruvbox; set background=dark; set number;
      set cursorline; set showmatch; set pastetoggle=<F2>; set mouse=a;
      nnoremap Q :Rexplore<CR>; inoremap jj <Esc>;
    '';
  };
  ...
  wayland.windowManager.hyprland = {
    ...
    extraConfig = ''
      ...
      bind = , F1, workspace, 1; bind = , F2, workspace, 2; bind = , F3, workspace, 3; bind = , F4, workspace, 4; bind = , F5, workspace, 5;
      bind = , F6, workspace, 6; bind = , F7, workspace, 7; bind = , F8, workspace, 8; bind = , F9, workspace, 9; bind = , F10, workspace, 10;
      bind = $mainMod, 1, movetoworkspace, 1; bind = $mainMod, 2, movetoworkspace, 2; bind = $mainMod, 3, movetoworkspace, 3;
      bind = $mainMod, 4, movetoworkspace, 4; bind = $mainMod, 5, movetoworkspace, 5; bind = $mainMod, 6, movetoworkspace, 6;
      bind = $mainMod, 7, movetoworkspace, 7; bind = $mainMod, 8, movetoworkspace, 8; bind = $mainMod, 9, movetoworkspace, 9;
